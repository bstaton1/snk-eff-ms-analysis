...
# USES THE EXTERNAL HYPERGEOMETRIC N ESTIMATE AS KNOWN DATA

# OUTPUT FROM THIS MODEL IS CALLED "fixN"
...
jags_data = list(
  ...
  # prior probability of each coefficient: defined in scenarios.csv and parameters.R
  w_prior = w_prior,
  
  # mark-recap information
  N = dat_train$N_est_hyper,
...
starttime = Sys.time()
cat("  (fixN) MCMC Started:   ", format(starttime), "\n")
post_fixN = jagsUI::jags.basic(
  data = jags_data,
...
unlink(jags_file)
stoptime = Sys.time()
cat("  (fixN) MCMC Elapsed:   ", format(stoptime - starttime, digits = 2), "\n")
cat("  -------------------------------------------\n")
